...
interface BountyHunterModeProps { username: string; onBack: () => void; adminAbuseEvents?: { event_type: string; expires_at: string }[]; touchscreenMode?: boolean; playerSkin?: string; }
...
// BOUNTY HUNTER: One enemy is marked as the bounty target (gold glow). Killing it gives 50 bonus points. Crimson/gold theme.
export const BountyHunterMode = ({ username, onBack, adminAbuseEvents = [], touchscreenMode = false, playerSkin = "#FFF3D6" }: BountyHunterModeProps) => {
  ...
  useEffect(() => {
    ...
    let time = 0, lastSpawn = 0, enemySpawnInterval = 2.0, bountyCount = 0;
    let last = performance.now();
    gameStateRef.current.enemies = enemies; gameStateRef.current.pickups = pickups;
    const rand = (min: number, max: number) => Math.random() * (max - min) + min;

    const spawnEnemy = (isBounty = false) => {
      ...
      enemies.push({ id: `e_${Date.now()}_${Math.random().toString(36).substr(2,6)}`, x, y, r: isBounty ? 20 : 16, speed: isBounty ? rand(30, 60) : rand(50, 90), hp: isBounty ? 150 : 70, maxHp: isBounty ? 150 : 70, color: isBounty ? "#F59E0B" : "#DC2626", stun: 0, lastShot: -1, isBounty });
    };
    ...
    const tryShoot = (t: number) => {
      ...
      if (weapon.isMelee) {
        if (mouse.down && t - player.lastMelee >= weapon.fireRate) {
          ...
          for (let i = enemies.length - 1; i >= 0; i--) { const e = enemies[i], dx = e.x - player.x, dy = e.y - player.y; if (Math.hypot(dx, dy) <= 50 && Math.abs(Math.atan2(dy, dx) - player.angle) < 0.5) { e.hp -= weapon.damage; e.stun = 0.6; spawnParticles(e.x, e.y, weapon.color, 12); if (e.hp <= 0) { const bonus = e.isBounty ? 50 : 10; spawnParticles(e.x, e.y, e.isBounty ? "#F59E0B" : "#DC2626", 25); if (e.isBounty) { bountyCount++; setBountiesCollected(bountyCount); toast.success(`💰 Bounty collected! +${bonus} points!`); } setScore(p => { const ns = p + bonus; player.score = ns; return ns; }); setKills(p => p + 1); if (Math.random() < 0.35) pickups.push({ x: e.x, y: e.y, r: 10, amt: 3, ttl: 18 }); enemies.splice(i, 1); } } }
        }
        return;
      }
    ...
    const loop = (now: number) => {
      ...
      if (player.hp <= 0 && !adminStateRef.current.godMode) {
        ...
        ctx.fillStyle = "#F59E0B"; ctx.font = "bold 48px sans-serif"; ctx.textAlign = "center"; ctx.fillText("HUNT OVER", W / 2, H / 2 - 30);
        ctx.fillStyle = "#fff"; ctx.font = "24px sans-serif"; ctx.fillText(`Bounties: ${bountyCount} • Score: ${score}`, W / 2, H / 2 + 20);
      ...
      for (let i = bullets.length - 1; i >= 0; i--) {
        ...
        for (let j = enemies.length - 1; j >= 0; j--) {
          ...
          if ((b.x - e.x) ** 2 + (b.y - e.y) ** 2 <= (b.r + e.r) ** 2) {
            ...
            if (e.hp <= 0) { const bonus = e.isBounty ? 50 : 10; spawnParticles(e.x, e.y, e.isBounty ? "#F59E0B" : "#DC2626", e.isBounty ? 30 : 16); if (e.isBounty) { bountyCount++; setBountiesCollected(bountyCount); toast.success(`💰 Bounty collected! +${bonus}!`); } setScore(p => { const ns = p + bonus; player.score = ns; return ns; }); setKills(p => p + 1); if (Math.random() < 0.35) pickups.push({ x: e.x, y: e.y, r: 10, amt: 3, ttl: 18 }); enemies.splice(j, 1); }
      ...
      for (const e of enemies) {
        ...
        if (d < 350 && time - e.lastShot >= 3.5) { e.lastShot = time; const ang = Math.atan2(player.y - e.y, player.x - e.x); enemyBullets.push({ x: e.x, y: e.y, vx: Math.cos(ang) * 200, vy: Math.sin(ang) * 200, r: 6, life: 3, dmg: 10, color: "#DC2626" }); }
      }
      ...
      // Spawn: every 5th enemy is a bounty
      if (time - lastSpawn > enemySpawnInterval) { lastSpawn = time; const isBounty = !enemies.some(e => e.isBounty); spawnEnemy(isBounty); if (enemySpawnInterval > 0.8) enemySpawnInterval *= 0.995; }

      // Draw
      ctx.clearRect(0, 0, W, H);
      const bg = ctx.createLinearGradient(0, 0, 0, H);
      bg.addColorStop(0, "#1a0505"); bg.addColorStop(1, "#0a0202");
      ctx.fillStyle = bg; ctx.fillRect(0, 0, W, H);
      ctx.save(); ctx.globalAlpha = 0.06; ctx.strokeStyle = "#DC2626";
      for (let x = 0; x < W; x += 40) { ctx.beginPath(); ctx.moveTo(x, 0); ctx.lineTo(x, H); ctx.stroke(); }
      for (let y = 0; y < H; y += 40) { ctx.beginPath(); ctx.moveTo(0, y); ctx.lineTo(W, y); ctx.stroke(); }
      ctx.restore();

      // Bounty indicator
      const bountyEnemy = enemies.find(e => e.isBounty);
      ctx.save();
      ctx.fillStyle = "rgba(245,158,11,0.2)"; ctx.fillRect(W / 2 - 100, 8, 200, 30);
      ctx.strokeStyle = "#F59E0B"; ctx.lineWidth = 1; ctx.strokeRect(W / 2 - 100, 8, 200, 30);
      ctx.fillStyle = "#F59E0B"; ctx.font = "bold 14px sans-serif"; ctx.textAlign = "center";
      ctx.fillText(bountyEnemy ? `💰 BOUNTY ACTIVE! • ${bountyCount} collected` : `💰 Bounties: ${bountyCount}`, W / 2, 28);
      ctx.restore();

      // Arrow pointing to bounty
      if (bountyEnemy) {
        const ang = Math.atan2(bountyEnemy.y - player.y, bountyEnemy.x - player.x);
        const arrowDist = 50;
        const ax = player.x + Math.cos(ang) * arrowDist, ay = player.y + Math.sin(ang) * arrowDist;
        ctx.save(); ctx.fillStyle = "#F59E0B"; ctx.globalAlpha = 0.8; ctx.translate(ax, ay); ctx.rotate(ang);
      ...
      for (const e of enemies) { ctx.save(); ctx.translate(e.x, e.y); if (e.isBounty) { ctx.shadowColor = "#F59E0B"; ctx.shadowBlur = 25; } ctx.fillStyle = e.color; ctx.beginPath(); ctx.arc(0, 0, e.r, 0, Math.PI * 2); ctx.fill(); ctx.shadowBlur = 0; const hpW = 28; ctx.fillStyle = "#333"; ctx.fillRect(-hpW / 2, -e.r - 12, hpW, 6); ctx.fillStyle = e.isBounty ? "#F59E0B" : "#DC2626"; ctx.fillRect(-hpW / 2, -e.r - 12, hpW * Math.max(0, e.hp / e.maxHp), 6); if (e.isBounty) { ctx.fillStyle = "#fff"; ctx.font = "bold 10px sans-serif"; ctx.textAlign = "center"; ctx.fillText("💰", 0, 4); } ctx.restore(); }
      ...
      if (isImmune) { ctx.strokeStyle = "#F59E0B"; ctx.lineWidth = 3; ctx.beginPath(); ctx.arc(0, 0, player.r + 5, 0, Math.PI * 2); ctx.stroke(); }
      ...
      ctx.save(); ctx.strokeStyle = "#F59E0B"; ctx.globalAlpha = 0.9; ctx.lineWidth = 1.5; ctx.beginPath(); ctx.moveTo(mouse.x - 10, mouse.y); ctx.lineTo(mouse.x + 10, mouse.y); ctx.moveTo(mouse.x, mouse.y - 10); ctx.lineTo(mouse.x, mouse.y + 10); ctx.stroke(); ctx.restore();
      gameLoopRef.current = requestAnimationFrame(loop);
    };

    spawnEnemy(true); // First enemy is a bounty
  ...
  return (
    <div className="relative">
      <div className="fixed left-4 top-4 bg-card/80 backdrop-blur-sm border border-amber-500/30 rounded-lg p-4 space-y-2">
        <div className="font-bold text-lg text-amber-400">💰 Bounty Hunter</div>
        <div className="text-sm text-muted-foreground space-y-1">
          <div><span className="text-amber-400 font-mono">WASD</span> move</div>
          <div><span className="text-amber-400 font-mono">Mouse</span> aim</div>
          <div><span className="text-amber-400 font-mono">LMB</span> shoot</div>
          {!WEAPONS[currentWeapon].isMelee && <div><span className="text-amber-400 font-mono">R</span> reload</div>}
          <div><span className="text-amber-400 font-mono">1-{unlockedWeapons.length}</span> weapons</div>
          <div className="text-amber-300 text-xs mt-2">Hunt the golden bounty target!</div>
        </div>
      </div>
      <div className="fixed right-4 top-4 bg-card/80 backdrop-blur-sm border border-amber-500/30 rounded-lg p-4 space-y-3 min-w-[180px]">
        <div className="text-center text-amber-400 font-bold">💰 Bounties: {bountiesCollected}</div>
        ...
        <div className="w-full bg-secondary rounded-full h-3 overflow-hidden"><div className="bg-gradient-to-r from-red-600 to-amber-400 h-full transition-all" style={{ width: `${(health / maxHealth) * 100}%` }} /></div>
        {spawnImmunity && <div className="flex items-center gap-2 text-amber-400 text-sm"><Shield className="w-4 h-4" /><span>Spawn Protection</span></div>}
        ...
        {!WEAPONS[currentWeapon].isMelee && (<><div className="flex justify-between items-center"><span className="text-sm text-muted-foreground">Ammo</span><span className="font-bold text-lg">{ammo}/{maxAmmo}</span></div><div className="w-full bg-secondary rounded-full h-2"><div className="bg-amber-400 h-full rounded-full" style={{ width: `${(ammo / maxAmmo) * 100}%` }} /></div></>)}
        <div className="pt-2 border-t border-amber-500/20"><div className="flex justify-between"><span className="text-sm text-muted-foreground">Score</span><span className="font-bold text-lg text-amber-400">{score}</span></div><div className="flex justify-between text-xs text-muted-foreground mt-1"><span>Kills: {kills}</span></div></div>
      </div>
      <div className="fixed bottom-4 left-1/2 -translate-x-1/2 bg-card/90 backdrop-blur-sm border border-amber-500/30 rounded-lg p-2 flex gap-2">
        {unlockedWeapons.map((weapon, index) => (<div key={weapon} className={`w-16 h-16 rounded-lg flex flex-col items-center justify-center cursor-pointer transition-all ${currentWeapon === weapon ? "bg-amber-500 text-black ring-2 ring-amber-400" : "bg-secondary hover:bg-secondary/80"}`} onClick={() => { if (playerRef.current) { playerRef.current.weapon = weapon; const wc = WEAPONS[weapon]; playerRef.current.ammo = wc.ammo; playerRef.current.maxAmmo = wc.maxAmmo; setCurrentWeapon(weapon); setAmmo(wc.ammo); setMaxAmmo(wc.maxAmmo); } }}><span className="text-xs opacity-70">{index + 1}</span><span className="text-xs font-medium text-center">{WEAPONS[weapon].name}</span></div>))}
      </div>
      <canvas ref={canvasRef} width={960} height={640} className="border-2 border-amber-500/30 rounded-lg shadow-2xl" />
