...
interface MedicModeProps { username: string; onBack: () => void; adminAbuseEvents?: { event_type: string; expires_at: string }[]; touchscreenMode?: boolean; playerSkin?: string; }
...
// MEDIC: Protect and heal 3 injured allies who wander around. If all die, you lose. White/cyan theme.
export const MedicMode = ({ username, onBack, adminAbuseEvents = [], touchscreenMode = false, playerSkin = "#FFF3D6" }: MedicModeProps) => {
  ...
  const [alliesAlive, setAlliesAlive] = useState(3);
  ...
  const handleCommand = useCallback((cmd: string) => { if (!hasPermission && !adminStateRef.current.active) return; if (cmd.startsWith("/godmode")) { adminStateRef.current.godMode = !adminStateRef.current.godMode; } else if (cmd.startsWith("/nuke")) { gameStateRef.current.enemies.length = 0; } else if (cmd.startsWith("/revive")) { revivePlayer(); } else if (cmd.startsWith("/give")) { setUnlockedWeapons([...WEAPON_ORDER]); } else if (cmd.startsWith("/infiniteammo")) { adminStateRef.current.infiniteAmmo = !adminStateRef.current.infiniteAmmo; } }, [hasPermission, revivePlayer]);
  ...
  useEffect(() => {
    ...
    let allies = [
      { x: 100, y: 100, hp: 100, maxHp: 100, speed: 40, angle: 0, dead: false },
      { x: W - 100, y: H - 100, hp: 100, maxHp: 100, speed: 40, angle: 0, dead: false },
      { x: W / 2, y: H / 2 + 100, hp: 100, maxHp: 100, speed: 40, angle: 0, dead: false }
    ];
    ...
    const spawnEnemy = () => {
      ...
      enemies.push({ id: `e_${Date.now()}_${Math.random().toString(36).substr(2,6)}`, x, y, r: 16, speed: rand(40, 80), hp: 60, maxHp: 60, color: "#60A5FA", stun: 0, lastShot: -1 });
    };
    ...
    const tryShoot = (t: number) => {
      ...
      if (weapon.isMelee) {
        if (mouse.down && t - player.lastMelee >= weapon.fireRate) {
          player.lastMelee = t; spawnParticles(player.x + Math.cos(player.angle) * 50, player.y + Math.sin(player.angle) * 50, weapon.color, 10);
          for (let i = enemies.length - 1; i >= 0; i--) { const e = enemies[i], dx = e.x - player.x, dy = e.y - player.y; if (Math.hypot(dx, dy) <= 50 && Math.abs(Math.atan2(dy, dx) - player.angle) < 0.5) { e.hp -= weapon.damage; e.stun = 0.6; spawnParticles(e.x, e.y, weapon.color, 12); if (e.hp <= 0) { spawnParticles(e.x, e.y, "#60A5FA", 20); setScore(p => { const ns = p + 10; player.score = ns; return ns; }); setKills(p => p + 1); if (Math.random() < 0.35) pickups.push({ x: e.x, y: e.y, r: 10, amt: 3, ttl: 18 }); enemies.splice(i, 1); } } }
        }
        return;
      }
    ...
    const loop = (now: number) => {
      ...
      if (player.hp <= 0 && !adminStateRef.current.godMode) {
        ...
        ctx.fillStyle = "#22D3EE"; ctx.font = "bold 48px sans-serif"; ctx.textAlign = "center"; ctx.fillText("MISSION FAILED", W / 2, H / 2 - 30);
        ctx.fillStyle = "#fff"; ctx.font = "24px sans-serif"; ctx.fillText(`Allies Saved: ${alliesAlive} • Score: ${score}`, W / 2, H / 2 + 20);
        ctx.font = "14px sans-serif"; ctx.fillStyle = "#aaa"; ctx.fillText("Use /revive or press Back", W / 2, H / 2 + 60);
        gameLoopRef.current = requestAnimationFrame(loop); return;
      }

      if (alliesAlive <= 0) {
        if (!gameOver) { setGameOver(true); saveProgress(scoreRef.current); toast.error("All allies have fallen!"); }
        ctx.fillStyle = "rgba(0,0,0,0.8)"; ctx.fillRect(0, 0, W, H);
        ctx.fillStyle = "#22D3EE"; ctx.font = "bold 48px sans-serif"; ctx.textAlign = "center"; ctx.fillText("ALL ALLIES LOST", W / 2, H / 2 - 30);
      ...
      // Allies wander and take damage
      let aliveCount = 0;
      for (const ally of allies) {
        if (ally.dead) continue;
        aliveCount++;
        // Wander
        ally.angle += rand(-0.1, 0.1);
        ally.x += Math.cos(ally.angle) * ally.speed * dt;
        ally.y += Math.sin(ally.angle) * ally.speed * dt;
        ally.x = Math.max(30, Math.min(W - 30, ally.x)); ally.y = Math.max(30, Math.min(H - 30, ally.y));
        if (ally.x <= 30 || ally.x >= W - 30 || ally.y <= 30 || ally.y >= H - 30) ally.angle += Math.PI;

        // Heal ally if player is close
        const dist = Math.hypot(player.x - ally.x, player.y - ally.y);
        if (dist < 60 && ally.hp < 100) {
          ally.hp = Math.min(100, ally.hp + 10 * dt);
          if (Math.random() < 0.1) spawnParticles(ally.x, ally.y, "#22C55E", 2);
        }
      }
      setAlliesAlive(aliveCount);

      for (let i = bullets.length - 1; i >= 0; i--) {
        const b = bullets[i]; b.x += b.vx * dt; b.y += b.vy * dt; b.life -= dt;
        if (b.life <= 0 || b.x < -50 || b.x > W + 50 || b.y < -50 || b.y > H + 50) { bullets.splice(i, 1); continue; }
        for (let j = enemies.length - 1; j >= 0; j--) { const e = enemies[j]; if ((b.x - e.x) ** 2 + (b.y - e.y) ** 2 <= (b.r + e.r) ** 2) { e.hp -= b.dmg; e.stun = 0.6; spawnParticles(b.x, b.y, "#FFF3D6", 8); if (e.hp <= 0) { spawnParticles(e.x, e.y, "#60A5FA", 16); setScore(p => { const ns = p + 10; player.score = ns; return ns; }); setKills(p => p + 1); if (Math.random() < 0.35) pickups.push({ x: e.x, y: e.y, r: 10, amt: 3, ttl: 18 }); enemies.splice(j, 1); } bullets.splice(i, 1); break; } }
      }

      const isImmune = spawnImmunityRef.current;
      for (let i = enemyBullets.length - 1; i >= 0; i--) { const b = enemyBullets[i]; b.x += b.vx * dt; b.y += b.vy * dt; b.life -= dt; if (b.life <= 0) { enemyBullets.splice(i, 1); continue; } if ((b.x - player.x) ** 2 + (b.y - player.y) ** 2 <= (b.r + player.r) ** 2) { if (!adminStateRef.current.godMode && !isImmune) { player.hp -= b.dmg; setHealth(Math.max(0, player.hp)); } enemyBullets.splice(i, 1); } }

      // Enemies target allies 70% of the time
      for (const e of enemies) {
        ...
        for (const ally of allies) {
          ...
          if (d < minDist * 0.7) { minDist = d; target = ally as any; }
        }
        const vx = target.x - e.x, vy = target.y - e.y;
        if (minDist > 0) { e.x += (vx / minDist) * e.speed * dt; e.y += (vy / minDist) * e.speed * dt; }
        
        // Damage target
        if (minDist < (target === player ? player.r : 15) + e.r) {
          if (target === player) { if (!adminStateRef.current.godMode && !isImmune) { player.hp -= 5 * dt; setHealth(Math.max(0, player.hp)); } }
          else { target.hp -= 15 * dt; if (target.hp <= 0) { target.dead = true; spawnParticles(target.x, target.y, "#EF4444", 30); toast.error("An ally has fallen!"); } }
        }
        if (minDist < 350 && time - e.lastShot >= 3.5) { e.lastShot = time; const ang = Math.atan2(target.y - e.y, target.x - e.x); enemyBullets.push({ x: e.x, y: e.y, vx: Math.cos(ang) * 200, vy: Math.sin(ang) * 200, r: 6, life: 3, dmg: 10, color: "#60A5FA" }); }
      }
      ...
      if (time - lastSpawn > enemySpawnInterval) { lastSpawn = time; spawnEnemy(); if (enemySpawnInterval > 0.8) enemySpawnInterval *= 0.995; }

      // Draw
      ctx.clearRect(0, 0, W, H);
      const bg = ctx.createLinearGradient(0, 0, 0, H); bg.addColorStop(0, "#081015"); bg.addColorStop(1, "#05080a");
      ctx.fillStyle = bg; ctx.fillRect(0, 0, W, H);
      ctx.save(); ctx.globalAlpha = 0.06; ctx.strokeStyle = "#22D3EE";
      ...
      // Draw Allies
      for (const ally of allies) {
        if (ally.dead) { ctx.save(); ctx.translate(ally.x, ally.y); ctx.fillStyle = "#333"; ctx.beginPath(); ctx.arc(0, 0, 15, 0, Math.PI * 2); ctx.fill(); ctx.restore(); continue; }
        ctx.save(); ctx.translate(ally.x, ally.y); ctx.rotate(ally.angle);
        ctx.fillStyle = "#22C55E"; ctx.beginPath(); ctx.arc(0, 0, 15, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = "#fff"; ctx.font = "bold 12px sans-serif"; ctx.textAlign = "center"; ctx.rotate(-ally.angle); ctx.fillText("✚", 0, 4);
        // HP bar
        ctx.fillStyle = "#333"; ctx.fillRect(-20, -25, 40, 4);
        ctx.fillStyle = ally.hp > 50 ? "#22C55E" : "#EF4444"; ctx.fillRect(-20, -25, 40 * (ally.hp / ally.maxHp), 4);
        ctx.restore();
      }

      // HUD
      ctx.save(); ctx.fillStyle = "rgba(34,211,238,0.15)"; ctx.fillRect(W / 2 - 100, 8, 200, 30); ctx.strokeStyle = "#22D3EE"; ctx.lineWidth = 1; ctx.strokeRect(W / 2 - 100, 8, 200, 30);
      ctx.fillStyle = "#22D3EE"; ctx.font = "bold 14px sans-serif"; ctx.textAlign = "center";
      ctx.fillText(`✚ Allies Alive: ${alliesAlive}/3`, W / 2, 28); ctx.restore();
      ...
      for (const e of enemies) { ctx.save(); ctx.translate(e.x, e.y); ctx.fillStyle = e.color; ctx.beginPath(); ctx.arc(0, 0, e.r, 0, Math.PI * 2); ctx.fill(); const hpW = 28; ctx.fillStyle = "#333"; ctx.fillRect(-hpW / 2, -e.r - 12, hpW, 6); ctx.fillStyle = "#60A5FA"; ctx.fillRect(-hpW / 2, -e.r - 12, hpW * Math.max(0, e.hp / e.maxHp), 6); ctx.restore(); }
      ...
      if (isImmune) { ctx.strokeStyle = "#22D3EE"; ctx.lineWidth = 3; ctx.beginPath(); ctx.arc(0, 0, player.r + 5, 0, Math.PI * 2); ctx.stroke(); }
      ...
      ctx.fillStyle = WEAPONS[player.weapon].color; if (WEAPONS[player.weapon].isMelee) ctx.fillRect(player.r - 2, -3, 25, 6); else ctx.fillRect(player.r - 2, -6, 18, 12);
      ctx.restore();
      ...
      ctx.save(); ctx.strokeStyle = "#22D3EE"; ctx.globalAlpha = 0.9; ctx.lineWidth = 1.5; ctx.beginPath(); ctx.moveTo(mouse.x - 10, mouse.y); ctx.lineTo(mouse.x + 10, mouse.y); ctx.moveTo(mouse.x, mouse.y - 10); ctx.lineTo(mouse.x, mouse.y + 10); ctx.stroke(); ctx.restore();
  ...
  return (
    <div className="relative">
      <div className="fixed left-4 top-4 bg-card/80 backdrop-blur-sm border border-cyan-500/30 rounded-lg p-4 space-y-2">
        <div className="font-bold text-lg text-cyan-400">✚ Medic</div>
        <div className="text-sm text-muted-foreground space-y-1">
          <div><span className="text-cyan-400 font-mono">WASD</span> move</div>
          <div><span className="text-cyan-400 font-mono">Mouse</span> aim</div>
          <div><span className="text-cyan-400 font-mono">LMB</span> shoot</div>
          {!WEAPONS[currentWeapon].isMelee && <div><span className="text-cyan-400 font-mono">R</span> reload</div>}
          <div><span className="text-cyan-400 font-mono">1-{unlockedWeapons.length}</span> weapons</div>
          <div className="text-cyan-300 text-xs mt-2">Stand near allies to heal them!</div>
        </div>
      </div>
      <div className="fixed right-4 top-4 bg-card/80 backdrop-blur-sm border border-cyan-500/30 rounded-lg p-4 space-y-3 min-w-[180px]">
        <div className="text-center text-cyan-400 font-bold">Allies: {alliesAlive}/3</div>
        ...
        <div className="w-full bg-secondary rounded-full h-3 overflow-hidden"><div className="bg-gradient-to-r from-red-600 to-cyan-400 h-full transition-all" style={{ width: `${(health / maxHealth) * 100}%` }} /></div>
        {spawnImmunity && <div className="flex items-center gap-2 text-cyan-400 text-sm"><Shield className="w-4 h-4" /><span>Spawn Protection</span></div>}
        ...
        {!WEAPONS[currentWeapon].isMelee && (<><div className="flex justify-between items-center"><span className="text-sm text-muted-foreground">Ammo</span><span className="font-bold text-lg">{ammo}/{maxAmmo}</span></div><div className="w-full bg-secondary rounded-full h-2"><div className="bg-cyan-400 h-full rounded-full" style={{ width: `${(ammo / maxAmmo) * 100}%` }} /></div></>)}
        <div className="pt-2 border-t border-cyan-500/20"><div className="flex justify-between"><span className="text-sm text-muted-foreground">Score</span><span className="font-bold text-lg text-cyan-400">{score}</span></div><div className="flex justify-between text-xs text-muted-foreground mt-1"><span>Kills: {kills}</span></div></div>
      </div>
      <div className="fixed bottom-4 left-1/2 -translate-x-1/2 bg-card/90 backdrop-blur-sm border border-cyan-500/30 rounded-lg p-2 flex gap-2">
        {unlockedWeapons.map((weapon, index) => (<div key={weapon} className={`w-16 h-16 rounded-lg flex flex-col items-center justify-center cursor-pointer transition-all ${currentWeapon === weapon ? "bg-cyan-500 text-black ring-2 ring-cyan-400" : "bg-secondary hover:bg-secondary/80"}`} onClick={() => { if (playerRef.current) { playerRef.current.weapon = weapon; const wc = WEAPONS[weapon]; playerRef.current.ammo = wc.ammo; playerRef.current.maxAmmo = wc.maxAmmo; setCurrentWeapon(weapon); setAmmo(wc.ammo); setMaxAmmo(wc.maxAmmo); } }}><span className="text-xs opacity-70">{index + 1}</span><span className="text-xs font-medium text-center">{WEAPONS[weapon].name}</span></div>))}
      </div>
      <canvas ref={canvasRef} width={960} height={640} className="border-2 border-cyan-500/30 rounded-lg shadow-2xl" />
