...
export const ZombieMode = ({ username, onBack, adminAbuseEvents = [], touchscreenMode = false, playerSkin = "#FFF3D6" }: ZombieModeProps) => {
  ...
  const checkPermissions = async () => {
    ...
    if (roleData) { setHasPermission(true); adminStateRef.current.active = true; }
  };
  ...
  useEffect(() => {
    ...
    let enemySpawnInterval = 1.2; // ZOMBIE: faster spawns
    let waveEnemiesKilled = 0, waveEnemiesNeeded = 15; // ZOMBIE: more enemies per wave
    ...
    const spawnZombie = () => {
      ...
      const enemyId = `zombie_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
      const cw = waveRef.current;
      // ZOMBIE: slower but more HP, don't shoot - they charge
      const isTank = Math.random() < 0.15 * cw;
      enemies.push({
        id: enemyId, x, y, r: isTank ? 22 : 16,
        speed: rand(25, 55) * (1 + cw * 0.05),
        hp: isTank ? 200 * (1 + cw * 0.2) : 80 * (1 + cw * 0.2),
        maxHp: isTank ? 200 * (1 + cw * 0.2) : 80 * (1 + cw * 0.2),
        color: isTank ? "#4B0082" : "#2D5A27",
        stun: 0, lastHit: 0, lastShot: -1, isZombie: true, isTank,
      });
    };
    ...
    const tryShoot = (t: number) => {
        ...
        if (mouse.down && t - player.lastMelee >= weapon.fireRate) {
          ...
          for (let i = enemies.length - 1; i >= 0; i--) {
            ...
            if (dist <= meleeRange && angleDiff < 0.5) {
              ...
              if (e.hp <= 0) { spawnParticles(e.x, e.y, "#2D5A27", 20); setScore(prev => { const ns = prev + (e.isTank ? 25 : 10); player.score = ns; return ns; }); setKills(prev => prev + 1); waveEnemiesKilled++; if (Math.random() < 0.4) pickups.push({ x: e.x, y: e.y, r: 10, amt: 2, ttl: 18 }); enemies.splice(i, 1); }
            }
          }
        }
        return;
      ...
      if (mouse.down && t - player.lastShot >= fireRate && (hasInfiniteAmmo || player.ammo > 0)) {
        ...
        for (let i = 0; i < bulletsToFire; i++) {
          ...
          const dmgMul = specialPowerRef.current === "double_damage" ? 2 : 1;
          bullets.push({ x: player.x + Math.cos(ang) * player.r * 1.6, y: player.y + Math.sin(ang) * player.r * 1.6, vx: Math.cos(finalAngle) * weapon.bulletSpeed, vy: Math.sin(finalAngle) * weapon.bulletSpeed, r: player.weapon === "sniper" ? 6 : 8, life: 2.5, dmg: weapon.damage * dmgMul, color: weapon.color });
        }
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key.toLowerCase() === "shift" && specialPowerRef.current === "teleport") {
        ...
        if (now - teleportCooldownRef.current >= 3000) { teleportCooldownRef.current = now; const mult = gameStateRef.current.mapBoundsMultiplier; player.x = Math.max(20, Math.min(W * mult - 20, player.x + Math.cos(player.angle) * 150)); player.y = Math.max(20, Math.min(H * mult - 20, player.y + Math.sin(player.angle) * 150)); spawnParticles(player.x, player.y, "#00FFFF", 20); }
      }
      if (e.key.toLowerCase() === "h") {
        try {
          const pendingPacks = JSON.parse(localStorage.getItem("pendingHealthPacks") || "[]");
          if (pendingPacks.length > 0) { if (player.hp >= (player.maxHp || 100)) { toast.info("Already at full HP!"); } else { const pack = pendingPacks.shift(); localStorage.setItem("pendingHealthPacks", JSON.stringify(pendingPacks)); player.hp = Math.min(player.maxHp || 100, player.hp + (pack.healAmount || 25)); setHealth(player.hp); toast.success(`+${pack.healAmount || 25} HP!`); spawnParticles(player.x, player.y, "#22c55e", 15); } }
          else { const equipped = JSON.parse(localStorage.getItem("equippedHealthPacks") || "[]"); if (equipped.length > 0) { if (player.hp >= (player.maxHp || 100)) { toast.info("Already at full HP!"); } else { const packId = equipped[0]; const ha: Record<string, number> = { small_health: 25, medium_health: 50, large_health: 100 }; const healAmount = ha[packId] || 25; player.hp = Math.min(player.maxHp || 100, player.hp + healAmount); setHealth(player.hp); toast.success(`+${healAmount} HP!`); spawnParticles(player.x, player.y, "#22c55e", 15); (async () => { try { const { data: { user: cu } } = await supabase.auth.getUser(); if (!cu) return; const { data: inv } = await supabase.from("player_inventory").select("*").eq("user_id", cu.id).eq("item_id", packId).eq("item_type", "health_pack").maybeSingle(); if (inv) { if (inv.quantity <= 1) { await supabase.from("player_inventory").delete().eq("user_id", cu.id).eq("item_id", packId); const ue = equipped.filter((id: string) => id !== packId); localStorage.setItem("equippedHealthPacks", JSON.stringify(ue)); } else { await supabase.from("player_inventory").update({ quantity: inv.quantity - 1 }).eq("user_id", cu.id).eq("item_id", packId); } } } catch (err) { console.error(err); } })(); } } else { toast.error("No health packs!"); } }
        } catch (err) { console.error(err); }
      }
      const keyNum = parseInt(e.key);
      if (keyNum >= 1 && keyNum <= unlockedWeapons.length) { const wpn = unlockedWeapons[keyNum - 1]; if (wpn) { player.weapon = wpn; const wc = WEAPONS[wpn]; player.ammo = wc.ammo; player.maxAmmo = wc.maxAmmo; setCurrentWeapon(wpn); setAmmo(player.ammo); setMaxAmmo(player.maxAmmo); } }
    };
    ...
    const loop = (now: number) => {
      ...
      if (player.hp <= 0 && !adminStateRef.current.godMode) {
        ...
        ctx.save(); ctx.fillStyle = "rgba(0,0,0,0.7)"; ctx.fillRect(0, 0, W, H); ctx.fillStyle = "#2D5A27"; ctx.font = "48px sans-serif"; ctx.textAlign = "center";
        ctx.fillText("BRAINS...", W / 2, H / 2 - 20); ctx.fillStyle = "#fff"; ctx.font = "24px sans-serif";
        ctx.fillText(`Wave: ${waveRef.current} | Zombies Killed: ${kills}`, W / 2, H / 2 + 30); ctx.restore();
        gameLoopRef.current = requestAnimationFrame(loop); return;
      }

      // ZOMBIE: Wave completion
      if (waveEnemiesKilled >= waveEnemiesNeeded) {
        waveRef.current++; setWave(waveRef.current); waveEnemiesKilled = 0;
        waveEnemiesNeeded = 15 + waveRef.current * 5;
        enemySpawnInterval = Math.max(0.4, 1.2 - waveRef.current * 0.08);
        toast.success(`🧟 Horde Wave ${waveRef.current}!`);
      ...
      if (dx !== 0 || dy !== 0) {
        ...
        const newX = player.x + dx * player.speed * sm * dt; const newY = player.y + dy * player.speed * sm * dt;
        const mult = gameStateRef.current.mapBoundsMultiplier;
        if (newX < 20 || newX > W * mult - 20 || newY < 20 || newY > H * mult - 20) expandMap();
        player.x = Math.max(20, Math.min(W * mult - 20, newX)); player.y = Math.max(20, Math.min(H * mult - 20, newY));
      }
      tryShoot(time);
      ...
      for (let i = bullets.length - 1; i >= 0; i--) {
        ...
        for (let j = enemies.length - 1; j >= 0; j--) {
          ...
          if (edx * edx + edy * edy <= (b.r + e.r) * (b.r + e.r)) {
            ...
            if (e.hp <= 0) { spawnParticles(e.x, e.y, "#2D5A27", 16); setScore(prev => { const ns = prev + (e.isTank ? 25 : 10); player.score = ns; return ns; }); setKills(prev => prev + 1); waveEnemiesKilled++; if (Math.random() < 0.4) pickups.push({ x: e.x, y: e.y, r: 10, amt: 2, ttl: 18 }); enemies.splice(j, 1); }
            bullets.splice(i, 1); break;
          }
        }
      }

      // ZOMBIE: No enemy bullets - zombies only charge
      ...
      for (let i = enemies.length - 1; i >= 0; i--) {
        ...
        let esm = 1; if (specialPowerRef.current === "slow_motion" && d < 200) esm = 0.5;
        if (d > 0) { e.x += (vx / d) * e.speed * esm * dt; e.y += (vy / d) * e.speed * esm * dt; }
        // ZOMBIE: Contact damage is higher
        if (d < player.r + e.r) { if (!adminStateRef.current.godMode && !isImmune) { player.hp -= (e.isTank ? 15 : 8) * dt; setHealth(Math.max(0, player.hp)); } }
      }

      // Pickups
      for (let i = pickups.length - 1; i >= 0; i--) {
        const p = pickups[i]; p.ttl -= dt; if (p.ttl <= 0) { pickups.splice(i, 1); continue; }
        if ((player.x - p.x) ** 2 + (player.y - p.y) ** 2 <= (player.r + p.r) ** 2) { if (!WEAPONS[player.weapon].isMelee) { player.ammo = Math.min(player.maxAmmo, player.ammo + p.amt); setAmmo(player.ammo); } spawnParticles(p.x, p.y, "#A6FFB3", 10); pickups.splice(i, 1); }
      }
      // Particles
      ...
      // Spawn
      if (time - lastSpawn > enemySpawnInterval) { lastSpawn = time; spawnZombie(); }
      gameStateRef.current.enemies = enemies; gameStateRef.current.pickups = pickups;

      // === DRAW ===
      ctx.clearRect(0, 0, W, H);
      // Dark green background
      ctx.save(); ctx.globalAlpha = 0.08; ctx.strokeStyle = "#1a3a1a";
      for (let x = 0; x < W; x += 40) { ctx.beginPath(); ctx.moveTo(x, 0); ctx.lineTo(x, H); ctx.stroke(); }
      for (let y = 0; y < H; y += 40) { ctx.beginPath(); ctx.moveTo(0, y); ctx.lineTo(W, y); ctx.stroke(); }
      ctx.restore();

      // Wave indicator
      ctx.save(); ctx.fillStyle = "#2D5A27"; ctx.font = "bold 20px sans-serif"; ctx.textAlign = "center";
      ctx.fillText(`🧟 HORDE WAVE ${waveRef.current}`, W / 2, 30);
      ctx.font = "14px sans-serif"; ctx.fillStyle = "#aaa";
      ctx.fillText(`${waveEnemiesKilled}/${waveEnemiesNeeded} zombies slain`, W / 2, 50); ctx.restore();
      ...
      for (const e of enemies) {
        ...
        const hpW = 28; ctx.fillStyle = "#333"; ctx.fillRect(-hpW / 2, -e.r - 12, hpW, 6); ctx.fillStyle = e.isTank ? "#4B0082" : "#2D5A27"; ctx.fillRect(-hpW / 2, -e.r - 12, hpW * Math.max(0, e.hp / (e.maxHp || 80)), 6); ctx.restore();
      }
  ...
  return (
    <div className="relative">
      <div className="fixed left-4 top-4 bg-card/80 backdrop-blur-sm border border-border rounded-lg p-4 space-y-2">
        <div className="font-bold text-lg">🧟 Zombie Mode</div>
      ...
      <div className="fixed right-4 top-4 bg-card/80 backdrop-blur-sm border border-border rounded-lg p-4 space-y-3 min-w-[180px]">
        ...
        <div className="flex justify-between items-center"><span className="text-sm text-muted-foreground">Horde Wave</span><span className="font-bold text-lg text-green-500">{wave}</span></div>
        ...
        {!WEAPONS[currentWeapon].isMelee && (<><div className="flex justify-between items-center"><span className="text-sm text-muted-foreground">Ammo</span><span className="font-bold text-lg">{ammo}/{maxAmmo}</span></div><div className="w-full bg-secondary rounded-full h-2"><div className="bg-primary h-full rounded-full transition-all duration-300" style={{ width: `${(ammo / maxAmmo) * 100}%` }} /></div></>)}
        <div className="pt-2 border-t border-border"><div className="flex justify-between items-center"><span className="text-sm text-muted-foreground">Score</span><span className="font-bold text-lg text-primary">{score}</span></div><div className="text-xs text-muted-foreground mt-1">Zombies Killed: {kills}</div></div>
      </div>
