...
const COMMANDS = [
  ...
  { cmd: "/shield", desc: "Add temporary shield (10s)" },
  ...
  { cmd: "/laser", desc: "Fire a laser beam that pierces enemies" },
  { cmd: "/slowmo [seconds]", desc: "Slow down time for X seconds" },
  { cmd: "/cloak", desc: "Become invisible for 10 seconds" },
  { cmd: "/magnet", desc: "Attract all pickups to player" },
  { cmd: "/doubledmg", desc: "Double damage for 30 seconds" },
  { cmd: "/killstreak", desc: "Show your current killstreak" },
  { cmd: "/weather [rain/snow/clear]", desc: "Change weather effect" },
...
export const AdminChat = ({ open, onOpenChange, onCommand, onShowOnlinePlayers }: AdminChatProps) => {
  ...
  const handleCommand = (cmd: string) => {
    ...
    const knownCommands = new Set(COMMANDS.map((entry) => entry.cmd));
