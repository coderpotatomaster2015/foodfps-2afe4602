...
interface DemolitionModeProps { username: string; onBack: () => void; adminAbuseEvents?: { event_type: string; expires_at: string }[]; touchscreenMode?: boolean; playerSkin?: string; }
...
// DEMOLITION: Destroy 3 enemy bases scattered around the map. Stand near a base for 5s to plant bomb. Red/orange explosion theme.
export const DemolitionMode = ({ username, onBack, adminAbuseEvents = [], touchscreenMode = false, playerSkin = "#FFF3D6" }: DemolitionModeProps) => {
  ...
  const [basesDestroyed, setBasesDestroyed] = useState(0);
  ...
  useEffect(() => {
    ...
    // 3 bases to destroy
    const bases = [
      { x: 150, y: 150, hp: 100, maxHp: 100, plantProgress: 0, destroyed: false },
      { x: W - 150, y: 150, hp: 100, maxHp: 100, plantProgress: 0, destroyed: false },
      { x: W / 2, y: H - 120, hp: 100, maxHp: 100, plantProgress: 0, destroyed: false },
    ];
    ...
    const spawnEnemy = () => {
      ...
      enemies.push({ id: `e_${Date.now()}_${Math.random().toString(36).substr(2,6)}`, x, y, r: 16, speed: rand(50, 90), hp: 70, maxHp: 70, color: "#EF4444", stun: 0, lastShot: -1 });
    };
    ...
    const tryShoot = (t: number) => {
      ...
      if (weapon.isMelee) {
        if (mouse.down && t - player.lastMelee >= weapon.fireRate) {
          ...
          for (let i = enemies.length - 1; i >= 0; i--) { const e = enemies[i], dx = e.x - player.x, dy = e.y - player.y; if (Math.hypot(dx, dy) <= 50 && Math.abs(Math.atan2(dy, dx) - player.angle) < 0.5) { e.hp -= weapon.damage; e.stun = 0.6; spawnParticles(e.x, e.y, weapon.color, 12); if (e.hp <= 0) { spawnParticles(e.x, e.y, "#EF4444", 20); setScore(p => { const ns = p + 10; player.score = ns; return ns; }); setKills(p => p + 1); if (Math.random() < 0.35) pickups.push({ x: e.x, y: e.y, r: 10, amt: 3, ttl: 18 }); enemies.splice(i, 1); } } }
        }
        return;
      }
    ...
    const loop = (now: number) => {
      ...
      if (player.hp <= 0 && !adminStateRef.current.godMode) {
        ...
        ctx.fillStyle = "#EF4444"; ctx.font = "bold 48px sans-serif"; ctx.textAlign = "center"; ctx.fillText("MISSION FAILED", W / 2, H / 2 - 30);
        ctx.fillStyle = "#fff"; ctx.font = "24px sans-serif"; ctx.fillText(`Bases: ${destroyed}/3 • Score: ${score}`, W / 2, H / 2 + 20);
      ...
      // Plant bombs on bases: stand near for 5s cumulative
      for (const base of bases) {
        if (base.destroyed) continue;
        const dist = Math.hypot(player.x - base.x, player.y - base.y);
        if (dist < 60) {
          base.plantProgress += dt;
          if (base.plantProgress >= 5) {
            base.destroyed = true; destroyed++;
            setBasesDestroyed(destroyed);
            setScore(p => { const ns = p + 100; player.score = ns; return ns; });
            spawnParticles(base.x, base.y, "#EF4444", 40);
            spawnParticles(base.x, base.y, "#F59E0B", 30);
            toast.success(`💥 Base destroyed! ${destroyed}/3`);
            if (destroyed >= 3 && !modeWon) { modeWon = true; saveProgress(score + 300); toast.success("🏆 All bases destroyed! +300 bonus!"); setScore(p => p + 300); }
          }
        }
      }
      ...
      for (const e of enemies) { if (e.stun > 0) { e.stun -= dt; continue; } const vx = player.x - e.x, vy = player.y - e.y, d = Math.hypot(vx, vy); if (d > 0) { e.x += (vx / d) * e.speed * dt; e.y += (vy / d) * e.speed * dt; } if (d < player.r + e.r && !adminStateRef.current.godMode && !isImmune) { player.hp -= 5 * dt; setHealth(Math.max(0, player.hp)); } if (d < 350 && time - e.lastShot >= 3.5) { e.lastShot = time; const ang = Math.atan2(player.y - e.y, player.x - e.x); enemyBullets.push({ x: e.x, y: e.y, vx: Math.cos(ang) * 200, vy: Math.sin(ang) * 200, r: 6, life: 3, dmg: 10, color: "#EF4444" }); } }
      ...
      const bg = ctx.createLinearGradient(0, 0, 0, H); bg.addColorStop(0, "#1a0800"); bg.addColorStop(1, "#0a0400");
      ctx.fillStyle = bg; ctx.fillRect(0, 0, W, H);
      ctx.save(); ctx.globalAlpha = 0.06; ctx.strokeStyle = "#EF4444";
      ...
      for (const base of bases) {
        ...
        if (base.destroyed) {
          ctx.globalAlpha = 0.3; ctx.fillStyle = "#444"; ctx.fillRect(-25, -20, 50, 40); ctx.fillStyle = "#888"; ctx.font = "20px sans-serif"; ctx.textAlign = "center"; ctx.fillText("💥", 0, 5);
        } else {
          ctx.fillStyle = "#EF444466"; ctx.beginPath(); ctx.arc(0, 0, 60, 0, Math.PI * 2); ctx.fill();
          ctx.fillStyle = "#EF4444"; ctx.fillRect(-25, -20, 50, 40);
          ctx.fillStyle = "#fff"; ctx.font = "bold 12px sans-serif"; ctx.textAlign = "center"; ctx.fillText("🏗️ BASE", 0, 5);
          // Plant progress bar
          if (base.plantProgress > 0) {
            ctx.fillStyle = "#333"; ctx.fillRect(-25, 25, 50, 6);
            ctx.fillStyle = "#F59E0B"; ctx.fillRect(-25, 25, 50 * (base.plantProgress / 5), 6);
            ctx.fillStyle = "#fff"; ctx.font = "9px sans-serif"; ctx.fillText(`${Math.round(base.plantProgress / 5 * 100)}%`, 0, 30);
          }
          // HP bar
          ctx.fillStyle = "#333"; ctx.fillRect(-25, -28, 50, 4);
          ctx.fillStyle = "#EF4444"; ctx.fillRect(-25, -28, 50 * (base.hp / base.maxHp), 4);
        }
        ctx.restore();
      }

      // HUD
      ctx.save(); ctx.fillStyle = "rgba(239,68,68,0.15)"; ctx.fillRect(W / 2 - 100, 8, 200, 30); ctx.strokeStyle = "#EF4444"; ctx.lineWidth = 1; ctx.strokeRect(W / 2 - 100, 8, 200, 30);
      ctx.fillStyle = modeWon ? "#22C55E" : "#EF4444"; ctx.font = "bold 14px sans-serif"; ctx.textAlign = "center";
      ctx.fillText(modeWon ? "🏆 ALL BASES DESTROYED!" : `💥 Bases: ${destroyed}/3`, W / 2, 28); ctx.restore();
      ...
      for (const e of enemies) { ctx.save(); ctx.translate(e.x, e.y); ctx.fillStyle = e.color; ctx.beginPath(); ctx.arc(0, 0, e.r, 0, Math.PI * 2); ctx.fill(); const hpW = 28; ctx.fillStyle = "#333"; ctx.fillRect(-hpW / 2, -e.r - 12, hpW, 6); ctx.fillStyle = "#EF4444"; ctx.fillRect(-hpW / 2, -e.r - 12, hpW * Math.max(0, e.hp / e.maxHp), 6); ctx.restore(); }
      ...
      if (isImmune) { ctx.strokeStyle = "#EF4444"; ctx.lineWidth = 3; ctx.beginPath(); ctx.arc(0, 0, player.r + 5, 0, Math.PI * 2); ctx.stroke(); }
      ...
      ctx.save(); ctx.strokeStyle = "#EF4444"; ctx.globalAlpha = 0.9; ctx.lineWidth = 1.5; ctx.beginPath(); ctx.moveTo(mouse.x - 10, mouse.y); ctx.lineTo(mouse.x + 10, mouse.y); ctx.moveTo(mouse.x, mouse.y - 10); ctx.lineTo(mouse.x, mouse.y + 10); ctx.stroke(); ctx.restore();
  ...
  return (
    <div className="relative">
      <div className="fixed left-4 top-4 bg-card/80 backdrop-blur-sm border border-red-500/30 rounded-lg p-4 space-y-2">
        <div className="font-bold text-lg text-red-400">💥 Demolition</div>
        <div className="text-sm text-muted-foreground space-y-1">
          <div><span className="text-red-400 font-mono">WASD</span> move</div>
          <div><span className="text-red-400 font-mono">Mouse</span> aim</div>
          <div><span className="text-red-400 font-mono">LMB</span> shoot</div>
          {!WEAPONS[currentWeapon].isMelee && <div><span className="text-red-400 font-mono">R</span> reload</div>}
          <div><span className="text-red-400 font-mono">1-{unlockedWeapons.length}</span> weapons</div>
          <div className="text-red-300 text-xs mt-2">Stand near bases to plant bombs!</div>
        </div>
      </div>
      <div className="fixed right-4 top-4 bg-card/80 backdrop-blur-sm border border-red-500/30 rounded-lg p-4 space-y-3 min-w-[180px]">
        <div className="text-center text-red-400 font-bold">💥 Bases: {basesDestroyed}/3</div>
        ...
        <div className="w-full bg-secondary rounded-full h-3 overflow-hidden"><div className="bg-gradient-to-r from-red-600 to-orange-400 h-full transition-all" style={{ width: `${(health / maxHealth) * 100}%` }} /></div>
        {spawnImmunity && <div className="flex items-center gap-2 text-red-400 text-sm"><Shield className="w-4 h-4" /><span>Spawn Protection</span></div>}
        ...
        {!WEAPONS[currentWeapon].isMelee && (<><div className="flex justify-between items-center"><span className="text-sm text-muted-foreground">Ammo</span><span className="font-bold text-lg">{ammo}/{maxAmmo}</span></div><div className="w-full bg-secondary rounded-full h-2"><div className="bg-red-400 h-full rounded-full" style={{ width: `${(ammo / maxAmmo) * 100}%` }} /></div></>)}
        <div className="pt-2 border-t border-red-500/20"><div className="flex justify-between"><span className="text-sm text-muted-foreground">Score</span><span className="font-bold text-lg text-red-400">{score}</span></div><div className="flex justify-between text-xs text-muted-foreground mt-1"><span>Kills: {kills}</span></div></div>
      </div>
      <div className="fixed bottom-4 left-1/2 -translate-x-1/2 bg-card/90 backdrop-blur-sm border border-red-500/30 rounded-lg p-2 flex gap-2">
        {unlockedWeapons.map((weapon, index) => (<div key={weapon} className={`w-16 h-16 rounded-lg flex flex-col items-center justify-center cursor-pointer transition-all ${currentWeapon === weapon ? "bg-red-500 text-white ring-2 ring-red-400" : "bg-secondary hover:bg-secondary/80"}`} onClick={() => { if (playerRef.current) { playerRef.current.weapon = weapon; const wc = WEAPONS[weapon]; playerRef.current.ammo = wc.ammo; playerRef.current.maxAmmo = wc.maxAmmo; setCurrentWeapon(weapon); setAmmo(wc.ammo); setMaxAmmo(wc.maxAmmo); } }}><span className="text-xs opacity-70">{index + 1}</span><span className="text-xs font-medium text-center">{WEAPONS[weapon].name}</span></div>))}
      </div>
      <canvas ref={canvasRef} width={960} height={640} className="border-2 border-red-500/30 rounded-lg shadow-2xl" />
