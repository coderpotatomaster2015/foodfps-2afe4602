...
export const LastManStandingMode = ({ username, onBack, adminAbuseEvents = [], touchscreenMode = false, playerSkin = "#FFF3D6" }: LastManStandingModeProps) => {
  ...
  useEffect(() => {
    ...
    // No respawning in LMS - shrinking zone
    let zoneRadius = 400;
    let zoneCenter = { x: W / 2, y: H / 2 };
    let zoneShrinkRate = 3; // pixels per second
    let alive = 15;

    gameStateRef.current.enemies = enemies; gameStateRef.current.pickups = pickups;
    const rand = (min: number, max: number) => Math.random() * (max - min) + min;

    // Spawn all combatants at start (no more spawning)
    for (let i = 0; i < 15; i++) {
      const angle = (i / 15) * Math.PI * 2;
      const dist = rand(150, 350);
      enemies.push({
        id: `e_${i}_${Date.now()}`, x: W / 2 + Math.cos(angle) * dist, y: H / 2 + Math.sin(angle) * dist,
        r: 14, speed: rand(40, 80), hp: 80, maxHp: 80,
        color: `hsl(${rand(0, 360)}, 60%, 50%)`, stun: 0, lastShot: -1,
        targetEnemy: true // They also fight each other
      });
    }
    for (let i = 0; i < 5; i++) pickups.push({ x: rand(200, W - 200), y: rand(200, H - 200), r: 10, amt: 5, ttl: 60 });
    ...
    const tryShoot = (t: number) => {
      ...
      if (weapon.isMelee) {
        if (mouse.down && t - player.lastMelee >= weapon.fireRate) {
          ...
          for (let i = enemies.length - 1; i >= 0; i--) { const e = enemies[i]; const dx = e.x - player.x, dy = e.y - player.y; const dist = Math.hypot(dx, dy); const angleDiff = Math.abs(Math.atan2(dy, dx) - player.angle); if (dist <= meleeRange && angleDiff < 0.5) { e.hp -= weapon.damage; e.stun = 0.6; spawnParticles(e.x, e.y, weapon.color, 12); if (e.hp <= 0) { spawnParticles(e.x, e.y, e.color, 20); setScore(prev => { const ns = prev + 20; player.score = ns; return ns; }); setKills(prev => prev + 1); alive--; setAliveCount(alive + 1); enemies.splice(i, 1); if (alive <= 0) toast.success("🏆 VICTORY ROYALE! Last one standing!"); } } }
        }
        return;
      }
    ...
    const loop = (now: number) => {
      ...
      // Shrink zone
      zoneRadius = Math.max(80, 400 - time * zoneShrinkRate);

      if (player.hp <= 0 && !adminStateRef.current.godMode) {
        if (!gameOver) { setGameOver(true); setPlacement(alive + 1); saveProgress(scoreRef.current); }
        ctx.save(); ctx.fillStyle = "rgba(0,0,0,0.85)"; ctx.fillRect(0, 0, W, H);
        ctx.fillStyle = "#FF4444"; ctx.font = "bold 48px sans-serif"; ctx.textAlign = "center"; ctx.fillText("ELIMINATED!", W / 2, H / 2 - 30);
        ctx.fillStyle = "#fff"; ctx.font = "24px sans-serif"; ctx.fillText(`#${alive + 1} • Kills: ${kills} • Score: ${score}`, W / 2, H / 2 + 20);
      ...
      // Zone damage
      const playerDist = Math.hypot(player.x - zoneCenter.x, player.y - zoneCenter.y);
      if (playerDist > zoneRadius && !adminStateRef.current.godMode && !spawnImmunityRef.current) {
        player.hp -= 10 * dt; setHealth(Math.max(0, player.hp));
      }
      ...
      for (let i = bullets.length - 1; i >= 0; i--) { const b = bullets[i]; b.x += b.vx * dt; b.y += b.vy * dt; b.life -= dt; if (b.life <= 0 || b.x < -50 || b.x > W + 50 || b.y < -50 || b.y > H + 50) { bullets.splice(i, 1); continue; } for (let j = enemies.length - 1; j >= 0; j--) { const e = enemies[j]; if ((b.x - e.x) ** 2 + (b.y - e.y) ** 2 <= (b.r + e.r) ** 2) { e.hp -= b.dmg; e.stun = 0.6; spawnParticles(b.x, b.y, "#FFF3D6", 8); if (e.hp <= 0) { spawnParticles(e.x, e.y, e.color, 16); setScore(prev => { const ns = prev + 20; player.score = ns; return ns; }); setKills(prev => prev + 1); alive--; setAliveCount(alive + 1); enemies.splice(j, 1); if (alive <= 0) toast.success("🏆 VICTORY ROYALE!"); } bullets.splice(i, 1); break; } } }

      const isImmune = spawnImmunityRef.current;
      for (let i = enemyBullets.length - 1; i >= 0; i--) { const b = enemyBullets[i]; b.x += b.vx * dt; b.y += b.vy * dt; b.life -= dt; if (b.life <= 0 || b.x < -50 || b.x > W + 50 || b.y < -50 || b.y > H + 50) { enemyBullets.splice(i, 1); continue; } if ((b.x - player.x) ** 2 + (b.y - player.y) ** 2 <= (b.r + player.r) ** 2) { if (!adminStateRef.current.godMode && !isImmune) { player.hp -= b.dmg; setHealth(Math.max(0, player.hp)); } enemyBullets.splice(i, 1); } }

      // Enemies fight each other AND the player
      for (let i = enemies.length - 1; i >= 0; i--) {
        const e = enemies[i];
        if (e.stun > 0) { e.stun -= dt; continue; }
        // Zone damage to enemies
        const eDist = Math.hypot(e.x - zoneCenter.x, e.y - zoneCenter.y);
        if (eDist > zoneRadius) { e.hp -= 8 * dt; if (e.hp <= 0) { spawnParticles(e.x, e.y, e.color, 12); alive--; setAliveCount(alive + 1); enemies.splice(i, 1); continue; } }
        // Find closest target (player or other enemy)
        let closestDist = Math.hypot(player.x - e.x, player.y - e.y);
        let targetX = player.x, targetY = player.y;
        for (const other of enemies) { if (other === e) continue; const d = Math.hypot(other.x - e.x, other.y - e.y); if (d < closestDist) { closestDist = d; targetX = other.x; targetY = other.y; } }
        const vx = targetX - e.x, vy = targetY - e.y; const d = Math.hypot(vx, vy);
        ...
        if (dToPlayer < 300 && time - e.lastShot >= 2.5) { e.lastShot = time; const ang = Math.atan2(player.y - e.y, player.x - e.x); enemyBullets.push({ x: e.x, y: e.y, vx: Math.cos(ang) * 220, vy: Math.sin(ang) * 220, r: 6, life: 3, dmg: 12, color: "#FF8844" }); }
      }
      ...
      ctx.beginPath(); ctx.arc(zoneCenter.x, zoneCenter.y, zoneRadius, 0, Math.PI * 2); ctx.fill();
      ctx.globalCompositeOperation = "source-over";
      ctx.strokeStyle = "rgba(255, 100, 100, 0.6)"; ctx.lineWidth = 3; ctx.beginPath(); ctx.arc(zoneCenter.x, zoneCenter.y, zoneRadius, 0, Math.PI * 2); ctx.stroke();
      ctx.restore();

      ctx.save(); ctx.globalAlpha = 0.04; ctx.strokeStyle = "#888";
      for (let x = 0; x < W; x += 40) { ctx.beginPath(); ctx.moveTo(x, 0); ctx.lineTo(x, H); ctx.stroke(); }
      for (let y = 0; y < H; y += 40) { ctx.beginPath(); ctx.moveTo(0, y); ctx.lineTo(W, y); ctx.stroke(); }
      ctx.restore();

      // Alive counter
      ctx.save(); ctx.fillStyle = "rgba(0,0,0,0.6)"; ctx.fillRect(W / 2 - 60, 8, 120, 30);
      ctx.fillStyle = "#fff"; ctx.font = "bold 16px sans-serif"; ctx.textAlign = "center"; ctx.fillText(`👤 ${alive + 1} Alive`, W / 2, 28);
      ctx.restore();
      ...
      for (const e of enemies) { ctx.save(); ctx.translate(e.x, e.y); ctx.fillStyle = e.color; ctx.beginPath(); ctx.arc(0, 0, e.r, 0, Math.PI * 2); ctx.fill(); const hpW = 28; ctx.fillStyle = "#333"; ctx.fillRect(-hpW / 2, -e.r - 12, hpW, 6); ctx.fillStyle = "#FF6B6B"; ctx.fillRect(-hpW / 2, -e.r - 12, hpW * Math.max(0, e.hp / e.maxHp), 6); ctx.restore(); }
      ...
      if (isImmune) { ctx.strokeStyle = "#fff"; ctx.lineWidth = 3; ctx.beginPath(); ctx.arc(0, 0, player.r + 5, 0, Math.PI * 2); ctx.stroke(); }
      ...
      ctx.save(); ctx.strokeStyle = "#fff"; ctx.globalAlpha = 0.9; ctx.lineWidth = 1.5; ctx.beginPath(); ctx.moveTo(mouse.x - 10, mouse.y); ctx.lineTo(mouse.x + 10, mouse.y); ctx.moveTo(mouse.x, mouse.y - 10); ctx.lineTo(mouse.x, mouse.y + 10); ctx.stroke(); ctx.restore();
  ...
  return (
    <div className="relative">
      <div className="fixed left-4 top-4 bg-card/80 backdrop-blur-sm border border-border rounded-lg p-4 space-y-2">
        <div className="font-bold text-lg text-white">💀 Last Man Standing</div>
        <div className="text-sm text-muted-foreground space-y-1">
          <div><span className="text-white font-mono">WASD</span> move</div>
          <div><span className="text-white font-mono">Mouse</span> aim</div>
          <div><span className="text-white font-mono">LMB</span> shoot</div>
          {!WEAPONS[currentWeapon].isMelee && <div><span className="text-white font-mono">R</span> reload</div>}
          <div className="text-xs text-red-300">Zone shrinks! No respawns!</div>
        </div>
      </div>
      <div className="fixed right-4 top-4 bg-card/80 backdrop-blur-sm border border-red-500/30 rounded-lg p-4 space-y-3 min-w-[180px]">
        <div className="text-center text-white font-bold">💀 LMS</div>
        <div className="text-center text-2xl font-bold text-red-400">👤 {aliveCount}</div>
        <div className="text-center text-xs text-muted-foreground">players alive</div>
        ...
        {spawnImmunity && <div className="flex items-center gap-2 text-white text-sm"><Shield className="w-4 h-4" /><span>Spawn Protection</span></div>}
        ...
        {!WEAPONS[currentWeapon].isMelee && (<><div className="flex justify-between items-center"><span className="text-sm text-muted-foreground">Ammo</span><span className="font-bold text-lg">{ammo}/{maxAmmo}</span></div><div className="w-full bg-secondary rounded-full h-2"><div className="bg-red-400 h-full rounded-full transition-all duration-300" style={{ width: `${(ammo / maxAmmo) * 100}%` }} /></div></>)}
        <div className="pt-2 border-t border-red-500/20"><div className="flex justify-between"><span className="text-sm text-muted-foreground">Score</span><span className="font-bold text-lg text-red-400">{score}</span></div><div className="flex justify-between text-xs text-muted-foreground"><span>Kills: {kills}</span></div></div>
      </div>
