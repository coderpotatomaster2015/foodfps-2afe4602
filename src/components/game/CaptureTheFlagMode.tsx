...
export const CaptureTheFlagMode = ({ username, onBack, adminAbuseEvents = [], touchscreenMode = false, playerSkin = "#FFF3D6" }: CaptureTheFlagModeProps) => {
  ...
  useEffect(() => {
    ...
    // Player is on blue team, spawns on left side
    const player = { x: 100, y: H / 2, r: 14, speed: 180, angle: 0, weapon: "pistol" as Weapon, lastShot: -1, lastMelee: -1, hp: 100, maxHp: 100, score: 0, ammo: 10, maxAmmo: 10, carryingFlag: false };
    ...
    // Flag positions
    const blueBase = { x: 60, y: H / 2 };
    const redBase = { x: W - 60, y: H / 2 };
    let redFlag = { x: redBase.x, y: redBase.y, taken: false };
    let blueFlag = { x: blueBase.x, y: blueBase.y, taken: false, takenBy: -1 };
    ...
    // Red team enemies that guard and try to capture blue flag
    const spawnRedEnemy = () => {
      const id = `e_${Date.now()}_${Math.random().toString(36).substr(2, 6)}`;
      enemies.push({
        id, x: W - rand(50, 150), y: rand(80, H - 80), r: 16,
        speed: rand(60, 100), hp: 80, maxHp: 80,
        color: "#FF4444", stun: 0, lastShot: -1,
        role: Math.random() < 0.3 ? "attacker" : "defender" // attackers go for blue flag
      });
    };

    const spawnPickup = () => { pickups.push({ x: rand(200, W - 200), y: rand(80, H - 80), r: 10, amt: 3, ttl: 20 }); };
          ...
          for (let i = enemies.length - 1; i >= 0; i--) {
            ...
            if (dist <= meleeRange && angleDiff < 0.5) {
              ...
              if (e.hp <= 0) {
                ...
                if (e.carryingFlag) { blueFlag.taken = false; blueFlag.x = e.x; blueFlag.y = e.y; toast.info("Blue flag dropped!"); }
                enemies.splice(i, 1);
              }
            }
          }
    ...
    for (let i = 0; i < 6; i++) spawnRedEnemy();
    ...
    const loop = (now: number) => {
      ...
      if (player.hp <= 0 && !adminStateRef.current.godMode) {
        ...
        ctx.fillStyle = "#4488FF"; ctx.font = "bold 48px sans-serif"; ctx.textAlign = "center";
        ctx.fillText("CAPTURED!", W / 2, H / 2 - 30);
        ctx.fillStyle = "#fff"; ctx.font = "24px sans-serif";
        ctx.fillText(`Blue: ${blueCaps} • Red: ${redCaps} • Score: ${score}`, W / 2, H / 2 + 20);
      ...
      if (dx !== 0 || dy !== 0) {
        ...
        const spd = player.speed * adminStateRef.current.speedMultiplier * (player.carryingFlag ? 0.8 : 1);
      ...
      // Flag pickup - player picks up red flag
      if (!redFlag.taken && Math.hypot(player.x - redFlag.x, player.y - redFlag.y) < 30) {
        redFlag.taken = true; player.carryingFlag = true; setHasFlag(true);
        toast.success("🏳️ You grabbed the Red flag! Return to base!");
      }
      // Flag capture - return to blue base with red flag
      if (player.carryingFlag && Math.hypot(player.x - blueBase.x, player.y - blueBase.y) < 40) {
        blueCaps++; setBlueCaptures(blueCaps);
        player.carryingFlag = false; setHasFlag(false);
        redFlag = { x: redBase.x, y: redBase.y, taken: false };
        setScore(prev => { const ns = prev + 100; player.score = ns; return ns; });
        toast.success(`🏆 FLAG CAPTURED! Blue: ${blueCaps}`);
        spawnParticles(blueBase.x, blueBase.y, "#4488FF", 30);
      }
      ...
      for (let i = bullets.length - 1; i >= 0; i--) {
        ...
        for (let j = enemies.length - 1; j >= 0; j--) {
          ...
          if ((b.x - e.x) ** 2 + (b.y - e.y) ** 2 <= (b.r + e.r) ** 2) {
            ...
            if (e.hp <= 0) {
              spawnParticles(e.x, e.y, "#FF4444", 16);
              setScore(prev => { const ns = prev + 10; player.score = ns; return ns; });
              setKills(prev => prev + 1);
              if (e.carryingFlag) { blueFlag.taken = false; blueFlag.x = e.x; blueFlag.y = e.y; toast.info("Blue flag dropped!"); }
              enemies.splice(j, 1);
      ...
      for (let i = enemies.length - 1; i >= 0; i--) {
        ...
        let targetX, targetY;
        if (e.role === "attacker" && !blueFlag.taken) {
          // Go for blue flag
          targetX = blueFlag.x; targetY = blueFlag.y;
          // Pick up blue flag
          if (Math.hypot(e.x - blueFlag.x, e.y - blueFlag.y) < 20 && !blueFlag.taken) {
            blueFlag.taken = true; blueFlag.takenBy = i; e.carryingFlag = true;
            toast.error("🚨 Red team grabbed your flag!");
          }
          // If carrying, return to red base
          if (e.carryingFlag) { targetX = redBase.x; targetY = redBase.y; }
          if (e.carryingFlag && Math.hypot(e.x - redBase.x, e.y - redBase.y) < 40) {
            redCaps++; setRedCaptures(redCaps);
            e.carryingFlag = false; blueFlag = { x: blueBase.x, y: blueBase.y, taken: false, takenBy: -1 };
            toast.error(`💀 Red captured your flag! Red: ${redCaps}`);
          }
        } else {
          // Defend / chase player
        ...
        const vx = targetX - e.x, vy = targetY - e.y;
        ...
        if (dToPlayer < player.r + e.r && !adminStateRef.current.godMode && !isImmune) { player.hp -= 5 * dt; setHealth(Math.max(0, player.hp)); }
        if (dToPlayer < 350 && time - e.lastShot >= 3.0) {
          e.lastShot = time;
          const ang = Math.atan2(player.y - e.y, player.x - e.x);
          enemyBullets.push({ x: e.x, y: e.y, vx: Math.cos(ang) * 200, vy: Math.sin(ang) * 200, r: 6, life: 3, dmg: 10, color: "#FF8844" });
        }
      }

      // Respawn enemies
      if (time - lastSpawn > 5 && enemies.length < 8) { lastSpawn = time; spawnRedEnemy(); }
      ...
      // Two-tone background: blue left, red right
      ctx.fillStyle = "#080818"; ctx.fillRect(0, 0, W / 2, H);
      ctx.fillStyle = "#180808"; ctx.fillRect(W / 2, 0, W / 2, H);
      // Center line
      ctx.save(); ctx.strokeStyle = "rgba(255,255,255,0.15)"; ctx.lineWidth = 2; ctx.setLineDash([10, 10]);
      ctx.beginPath(); ctx.moveTo(W / 2, 0); ctx.lineTo(W / 2, H); ctx.stroke(); ctx.setLineDash([]); ctx.restore();
      // Grid
      ...
      // Blue base
      ctx.save();
      ctx.fillStyle = "rgba(68, 136, 255, 0.15)"; ctx.beginPath(); ctx.arc(blueBase.x, blueBase.y, 40, 0, Math.PI * 2); ctx.fill();
      ctx.strokeStyle = "#4488FF"; ctx.lineWidth = 2; ctx.stroke();
      ctx.fillStyle = "#4488FF"; ctx.font = "bold 12px sans-serif"; ctx.textAlign = "center"; ctx.fillText("BLUE BASE", blueBase.x, blueBase.y - 50);
      ctx.restore();

      // Red base
      ctx.save();
      ctx.fillStyle = "rgba(255, 68, 68, 0.15)"; ctx.beginPath(); ctx.arc(redBase.x, redBase.y, 40, 0, Math.PI * 2); ctx.fill();
      ctx.strokeStyle = "#FF4444"; ctx.lineWidth = 2; ctx.stroke();
      ctx.fillStyle = "#FF4444"; ctx.font = "bold 12px sans-serif"; ctx.textAlign = "center"; ctx.fillText("RED BASE", redBase.x, redBase.y - 50);
      ctx.restore();

      // Flags
      if (!redFlag.taken) {
        ctx.save(); ctx.translate(redFlag.x, redFlag.y);
        ctx.fillStyle = "#FF4444"; ctx.fillRect(-3, -20, 6, 20); ctx.fillRect(3, -20, 15, 10);
        ctx.restore();
      }
      if (!blueFlag.taken) {
        ctx.save(); ctx.translate(blueFlag.x, blueFlag.y);
        ctx.fillStyle = "#4488FF"; ctx.fillRect(-3, -20, 6, 20); ctx.fillRect(3, -20, 15, 10);
        ctx.restore();
      }

      // Score banner
      ctx.save();
      ctx.fillStyle = "rgba(0,0,0,0.5)"; ctx.fillRect(W / 2 - 100, 8, 200, 34);
      ctx.fillStyle = "#4488FF"; ctx.font = "bold 16px sans-serif"; ctx.textAlign = "right"; ctx.fillText(`${blueCaps}`, W / 2 - 10, 30);
      ctx.fillStyle = "#fff"; ctx.textAlign = "center"; ctx.fillText("vs", W / 2, 30);
      ctx.fillStyle = "#FF4444"; ctx.textAlign = "left"; ctx.fillText(`${redCaps}`, W / 2 + 10, 30);
      ctx.restore();
      ...
      for (const e of enemies) {
        ...
        if (e.carryingFlag) { ctx.fillStyle = "#4488FF"; ctx.fillRect(-3, -e.r - 18, 6, 14); ctx.fillRect(3, -e.r - 18, 10, 7); }
        const hpW = 28;
        ctx.fillStyle = "#333"; ctx.fillRect(-hpW / 2, -e.r - 12, hpW, 6);
        ctx.fillStyle = "#FF4444"; ctx.fillRect(-hpW / 2, -e.r - 12, hpW * Math.max(0, e.hp / e.maxHp), 6);
        ctx.restore();
      }
      ...
      if (isImmune) { ctx.strokeStyle = "#4488FF"; ctx.lineWidth = 3; ctx.beginPath(); ctx.arc(0, 0, player.r + 5, 0, Math.PI * 2); ctx.stroke(); }
      ...
      ctx.restore();
      // Draw flag on player if carrying
      if (player.carryingFlag) {
        ctx.save(); ctx.translate(player.x + 5, player.y - 20);
        ctx.fillStyle = "#FF4444"; ctx.fillRect(-2, -10, 4, 12); ctx.fillRect(2, -10, 10, 6);
        ctx.restore();
      }
      ...
      // Crosshair
      ctx.save(); ctx.strokeStyle = "#4488FF"; ctx.globalAlpha = 0.9; ctx.lineWidth = 1.5;
  ...
  return (
    <div className="relative">
      <div className="fixed left-4 top-4 bg-card/80 backdrop-blur-sm border border-border rounded-lg p-4 space-y-2">
        <div className="font-bold text-lg text-blue-400">🏳️ Capture the Flag</div>
        <div className="text-sm text-muted-foreground space-y-1">
          <div><span className="text-blue-400 font-mono">WASD</span> move</div>
          <div><span className="text-blue-400 font-mono">Mouse</span> aim</div>
          <div><span className="text-blue-400 font-mono">LMB</span> shoot</div>
          {!WEAPONS[currentWeapon].isMelee && <div><span className="text-blue-400 font-mono">R</span> reload</div>}
          <div><span className="text-blue-400 font-mono">1-{unlockedWeapons.length}</span> weapons</div>
        </div>
      </div>

      <div className="fixed right-4 top-4 bg-card/80 backdrop-blur-sm border border-blue-500/30 rounded-lg p-4 space-y-3 min-w-[180px]">
        <div className="text-center font-bold">
          <span className="text-blue-400">{blueCaptures}</span>
          <span className="text-muted-foreground mx-2">vs</span>
          <span className="text-red-400">{redCaptures}</span>
        </div>
        {hasFlag && <div className="text-center text-yellow-400 font-bold animate-pulse">🏳️ CARRYING FLAG!</div>}
        <div className="flex justify-between items-center">
          <span className="text-sm text-muted-foreground">Health</span>
          <span className="font-bold text-lg">{Math.round(health)}</span>
        </div>
        <div className="w-full bg-secondary rounded-full h-3 overflow-hidden">
          <div className="bg-gradient-to-r from-blue-600 to-blue-400 h-full transition-all duration-300" style={{ width: `${(health / maxHealth) * 100}%` }} />
        </div>
        {spawnImmunity && <div className="flex items-center gap-2 text-blue-400 text-sm"><Shield className="w-4 h-4" /><span>Spawn Protection</span></div>}
        <div className="flex justify-between items-center">
        ...
        {!WEAPONS[currentWeapon].isMelee && (
          <>
            ...
            <div className="w-full bg-secondary rounded-full h-2"><div className="bg-blue-400 h-full rounded-full transition-all duration-300" style={{ width: `${(ammo / maxAmmo) * 100}%` }} /></div>
          </>
        )}
        <div className="pt-2 border-t border-blue-500/20">
          <div className="flex justify-between"><span className="text-sm text-muted-foreground">Score</span><span className="font-bold text-lg text-blue-400">{score}</span></div>
          <div className="flex justify-between text-xs text-muted-foreground"><span>Kills: {kills}</span></div>
        </div>
      </div>

      <div className="fixed bottom-4 left-1/2 -translate-x-1/2 bg-card/90 backdrop-blur-sm border border-blue-500/30 rounded-lg p-2 flex gap-2">
        {unlockedWeapons.map((weapon, index) => (
          <div key={weapon} className={`w-16 h-16 rounded-lg flex flex-col items-center justify-center cursor-pointer transition-all ${currentWeapon === weapon ? "bg-blue-600 text-white ring-2 ring-blue-400" : "bg-secondary hover:bg-secondary/80"}`}
