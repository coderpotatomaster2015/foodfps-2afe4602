...
interface TagModeProps { username: string; onBack: () => void; adminAbuseEvents?: { event_type: string; expires_at: string }[]; touchscreenMode?: boolean; playerSkin?: string; }
...
// TAG MODE: You're "it" — touch enemies to tag them and earn points. When tagged back you lose HP. Yellow/green theme.
export const TagMode = ({ username, onBack, adminAbuseEvents = [], touchscreenMode = false, playerSkin = "#FFF3D6" }: TagModeProps) => {
  ...
  const handleCommand = useCallback((cmd: string) => { if (!hasPermission && !adminStateRef.current.active) return; if (cmd.startsWith("/godmode")) { adminStateRef.current.godMode = !adminStateRef.current.godMode; } else if (cmd.startsWith("/nuke")) { gameStateRef.current.enemies.length = 0; } else if (cmd.startsWith("/revive")) { revivePlayer(); } else if (cmd.startsWith("/give")) { setUnlockedWeapons([...WEAPON_ORDER]); toast.success("All weapons unlocked!"); } else if (cmd.startsWith("/infiniteammo")) { adminStateRef.current.infiniteAmmo = !adminStateRef.current.infiniteAmmo; } }, [hasPermission, revivePlayer]);
  ...
  useEffect(() => {
    ...
    const player = { x: W / 2, y: H / 2, r: 14, speed: 200, angle: 0, weapon: "pistol" as Weapon, lastShot: -1, lastMelee: -1, hp: 100, maxHp: 100, score: 0, ammo: 10, maxAmmo: 10, isIt: true, tagCooldown: 0 };
    playerRef.current = player;

    let bullets: any[] = [], enemyBullets: any[] = [], enemies: any[] = [], pickups: any[] = [], particles: any[] = [];
    let time = 0, lastSpawn = 0, enemySpawnInterval = 3.0, tagCount = 0;
    let last = performance.now();
    gameStateRef.current.enemies = enemies; gameStateRef.current.pickups = pickups;
    const rand = (min: number, max: number) => Math.random() * (max - min) + min;

    // Runners that flee from player (when player is "it"), or chase (when not "it")
    const spawnEnemy = () => {
      ...
      enemies.push({ id: `e_${Date.now()}_${Math.random().toString(36).substr(2,6)}`, x, y, r: 15, speed: rand(60, 120), hp: 80, maxHp: 80, color: "#FBBF24", stun: 0, lastShot: -1, tagged: false });
    };
    ...
    const tryShoot = (t: number) => {
      ...
      if (weapon.isMelee) {
        if (mouse.down && t - player.lastMelee >= weapon.fireRate) {
          ...
          for (let i = enemies.length - 1; i >= 0; i--) {
            ...
            if (Math.hypot(dx, dy) <= 50 && Math.abs(Math.atan2(dy, dx) - player.angle) < 0.5) {
              e.hp -= weapon.damage; e.stun = 0.6; spawnParticles(e.x, e.y, weapon.color, 12);
              if (e.hp <= 0) { spawnParticles(e.x, e.y, "#FBBF24", 20); setScore(p => { const ns = p + 10; player.score = ns; return ns; }); setKills(p => p + 1); if (Math.random() < 0.35) pickups.push({ x: e.x, y: e.y, r: 10, amt: 3, ttl: 18 }); enemies.splice(i, 1); }
            }
          }
        }
        return;
      ...
      if (mouse.down && t - player.lastShot >= fireRate && (hasInfinite || player.ammo > 0)) {
        ...
        for (let i = 0; i < count; i++) {
          ...
          bullets.push({ x: player.x + Math.cos(player.angle) * player.r * 1.6, y: player.y + Math.sin(player.angle) * player.r * 1.6, vx: Math.cos(ang) * weapon.bulletSpeed, vy: Math.sin(ang) * weapon.bulletSpeed, r: 8, life: 2.5, dmg: weapon.damage, color: weapon.color });
        }
    ...
    const loop = (now: number) => {
      ...
      if (player.hp <= 0 && !adminStateRef.current.godMode) {
        ...
        ctx.fillStyle = "#FBBF24"; ctx.font = "bold 48px sans-serif"; ctx.textAlign = "center"; ctx.fillText("TAGGED OUT!", W / 2, H / 2 - 30);
        ctx.fillStyle = "#fff"; ctx.font = "24px sans-serif"; ctx.fillText(`Tags: ${tagCount} • Score: ${score}`, W / 2, H / 2 + 20);
      ...
      for (let i = bullets.length - 1; i >= 0; i--) {
        ...
        for (let j = enemies.length - 1; j >= 0; j--) {
          ...
          if ((b.x - e.x) ** 2 + (b.y - e.y) ** 2 <= (b.r + e.r) ** 2) {
            ...
            if (e.hp <= 0) { spawnParticles(e.x, e.y, "#FBBF24", 16); setScore(p => { const ns = p + 10; player.score = ns; return ns; }); setKills(p => p + 1); if (Math.random() < 0.35) pickups.push({ x: e.x, y: e.y, r: 10, amt: 3, ttl: 18 }); enemies.splice(j, 1); }
      ...
      // Enemies: when player is "it", they RUN AWAY. When not "it", they CHASE.
      for (const e of enemies) {
        if (e.stun > 0) { e.stun -= dt; continue; }
        const vx = player.x - e.x, vy = player.y - e.y, d = Math.hypot(vx, vy);
        if (player.isIt) {
          // Run away from player
          if (d > 0 && d < 250) { e.x -= (vx / d) * e.speed * 1.2 * dt; e.y -= (vy / d) * e.speed * 1.2 * dt; }
          else if (d > 0) { e.x += rand(-1, 1) * e.speed * 0.5 * dt; e.y += rand(-1, 1) * e.speed * 0.5 * dt; }
        } else {
          // Chase player
          if (d > 0) { e.x += (vx / d) * e.speed * dt; e.y += (vy / d) * e.speed * dt; }
          if (d < player.r + e.r && !adminStateRef.current.godMode && !isImmune) {
            // Enemy tags player — player loses HP and becomes "it" again
            player.hp -= 15; setHealth(Math.max(0, player.hp));
            player.isIt = true; setIsIt(true);
            toast.info("🏷️ You got tagged! You're IT again!");
            spawnParticles(player.x, player.y, "#EF4444", 15);
          }
        }
        // Tag mechanic: player touches enemy while "it"
        if (player.isIt && d < player.r + e.r + 5 && player.tagCooldown <= 0) {
          e.tagged = true; e.color = "#22C55E"; e.stun = 1.5;
          tagCount++; setTagged(tagCount);
          setScore(p => { const ns = p + 20; player.score = ns; return ns; });
          spawnParticles(e.x, e.y, "#22C55E", 20);
          player.tagCooldown = 0.5;
          // After tagging 5, player is no longer "it" for 5 seconds
          if (tagCount % 5 === 0) { player.isIt = false; setIsIt(false); toast.success("🏃 Freedom! You're NOT it for 5 seconds!"); setTimeout(() => { player.isIt = true; setIsIt(true); toast.info("🏷️ You're IT again!"); }, 5000); }
        }
        // Enemies shoot
        if (!player.isIt && d < 300 && time - e.lastShot >= 4.0) {
          e.lastShot = time;
          const ang = Math.atan2(player.y - e.y, player.x - e.x);
          enemyBullets.push({ x: e.x, y: e.y, vx: Math.cos(ang) * 180, vy: Math.sin(ang) * 180, r: 6, life: 3, dmg: 8, color: "#F59E0B" });
        }
        e.x = Math.max(16, Math.min(W - 16, e.x)); e.y = Math.max(16, Math.min(H - 16, e.y));
      }
      ...
      if (time - lastSpawn > enemySpawnInterval && enemies.length < 12) { lastSpawn = time; spawnEnemy(); }

      // Draw
      ctx.clearRect(0, 0, W, H);
      const bg = ctx.createLinearGradient(0, 0, 0, H);
      bg.addColorStop(0, "#1a1800"); bg.addColorStop(1, "#0d0b00");
      ctx.fillStyle = bg; ctx.fillRect(0, 0, W, H);
      ctx.save(); ctx.globalAlpha = 0.06; ctx.strokeStyle = "#FBBF24";
      ...
      // IT indicator
      ctx.save();
      ctx.fillStyle = player.isIt ? "rgba(251,191,36,0.2)" : "rgba(34,197,94,0.2)";
      ctx.fillRect(W / 2 - 80, 8, 160, 30);
      ctx.strokeStyle = player.isIt ? "#FBBF24" : "#22C55E"; ctx.lineWidth = 1; ctx.strokeRect(W / 2 - 80, 8, 160, 30);
      ctx.fillStyle = player.isIt ? "#FBBF24" : "#22C55E"; ctx.font = "bold 14px sans-serif"; ctx.textAlign = "center";
      ctx.fillText(player.isIt ? `🏷️ YOU'RE IT! Tags: ${tagCount}` : `🏃 FREE! Tags: ${tagCount}`, W / 2, 28);
      ctx.restore();
      ...
      for (const e of enemies) { ctx.save(); ctx.translate(e.x, e.y); if (e.tagged) { ctx.shadowColor = "#22C55E"; ctx.shadowBlur = 10; } ctx.fillStyle = e.color; ctx.beginPath(); ctx.arc(0, 0, e.r, 0, Math.PI * 2); ctx.fill(); ctx.shadowBlur = 0; const hpW = 28; ctx.fillStyle = "#333"; ctx.fillRect(-hpW / 2, -e.r - 12, hpW, 6); ctx.fillStyle = "#FBBF24"; ctx.fillRect(-hpW / 2, -e.r - 12, hpW * Math.max(0, e.hp / e.maxHp), 6); ctx.restore(); }
      ...
      if (player.isIt) { ctx.shadowColor = "#FBBF24"; ctx.shadowBlur = 20; }
      if (isImmune) { ctx.strokeStyle = "#FBBF24"; ctx.lineWidth = 3; ctx.beginPath(); ctx.arc(0, 0, player.r + 5, 0, Math.PI * 2); ctx.stroke(); }
      ...
      ctx.save(); ctx.strokeStyle = player.isIt ? "#FBBF24" : "#22C55E"; ctx.globalAlpha = 0.9; ctx.lineWidth = 1.5;
  ...
  return (
    <div className="relative">
      <div className="fixed left-4 top-4 bg-card/80 backdrop-blur-sm border border-yellow-500/30 rounded-lg p-4 space-y-2">
        <div className="font-bold text-lg text-yellow-400">🏷️ Tag</div>
        <div className="text-sm text-muted-foreground space-y-1">
          ...
          <div><span className="text-yellow-400 font-mono">1-{unlockedWeapons.length}</span> weapons</div>
          <div className="text-yellow-300 text-xs mt-2">{isIt ? "Touch enemies to tag them!" : "Run! They're chasing you!"}</div>
        </div>
      </div>
      <div className="fixed right-4 top-4 bg-card/80 backdrop-blur-sm border border-yellow-500/30 rounded-lg p-4 space-y-3 min-w-[180px]">
        <div className={`text-center font-bold ${isIt ? "text-yellow-400" : "text-green-400"}`}>{isIt ? "🏷️ YOU'RE IT!" : "🏃 FREE!"}</div>
        <div className="text-center text-lg font-bold text-yellow-400">Tags: {tagged}</div>
        ...
        <div className="w-full bg-secondary rounded-full h-3 overflow-hidden"><div className="bg-gradient-to-r from-red-500 to-yellow-400 h-full transition-all" style={{ width: `${(health / maxHealth) * 100}%` }} /></div>
        ...
        {!WEAPONS[currentWeapon].isMelee && (
          <>
            ...
            <div className="w-full bg-secondary rounded-full h-2"><div className="bg-yellow-400 h-full rounded-full" style={{ width: `${(ammo / maxAmmo) * 100}%` }} /></div>
          </>
        )}
        <div className="pt-2 border-t border-yellow-500/20">
          <div className="flex justify-between"><span className="text-sm text-muted-foreground">Score</span><span className="font-bold text-lg text-yellow-400">{score}</span></div>
      ...
      <div className="fixed bottom-4 left-1/2 -translate-x-1/2 bg-card/90 backdrop-blur-sm border border-yellow-500/30 rounded-lg p-2 flex gap-2">
        {unlockedWeapons.map((weapon, index) => (
          <div key={weapon} className={`w-16 h-16 rounded-lg flex flex-col items-center justify-center cursor-pointer transition-all ${currentWeapon === weapon ? "bg-yellow-500 text-black ring-2 ring-yellow-400" : "bg-secondary hover:bg-secondary/80"}`}
