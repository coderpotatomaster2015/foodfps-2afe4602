...
export const KingOfTheHillMode = ({ username, onBack, adminAbuseEvents = [], touchscreenMode = false, playerSkin = "#FFF3D6" }: KingOfTheHillModeProps) => {
  ...
  const [hillOwner, setHillOwner] = useState<"player" | "enemy" | "none">("none");
  ...
  useEffect(() => {
    ...
    const player = { x: W / 4, y: H / 2, r: 14, speed: 180, angle: 0, weapon: "pistol" as Weapon, lastShot: -1, lastMelee: -1, hp: 100, maxHp: 100, score: 0, ammo: 10, maxAmmo: 10 };
    playerRef.current = player;

    let bullets: any[] = [], enemyBullets: any[] = [], enemies: any[] = [], pickups: any[] = [], particles: any[] = [];
    let time = 0, lastSpawn = 0, last = performance.now();
    let playerHillTime = 0, enemyHillTime = 0;
    const hillCenter = { x: W / 2, y: H / 2 };
    const hillRadius = 80;
    const winTime = 60; // 60 seconds on hill to win
    ...
    const spawnEnemy = () => {
      ...
      enemies.push({ id: `e_${Date.now()}_${Math.random().toString(36).substr(2, 6)}`, x, y, r: 16, speed: rand(60, 110), hp: 80, maxHp: 80, color: "#FFD700", stun: 0, lastShot: -1 });
    };
    ...
    const tryShoot = (t: number) => {
      ...
      if (weapon.isMelee) {
        if (mouse.down && t - player.lastMelee >= weapon.fireRate) {
          ...
          for (let i = enemies.length - 1; i >= 0; i--) { const e = enemies[i]; const dx = e.x - player.x, dy = e.y - player.y; const dist = Math.hypot(dx, dy); const angleDiff = Math.abs(Math.atan2(dy, dx) - player.angle); if (dist <= meleeRange && angleDiff < 0.5) { e.hp -= weapon.damage; e.stun = 0.6; spawnParticles(e.x, e.y, weapon.color, 12); if (e.hp <= 0) { spawnParticles(e.x, e.y, e.color, 16); setScore(prev => { const ns = prev + 10; player.score = ns; return ns; }); setKills(prev => prev + 1); enemies.splice(i, 1); } } }
        }
        return;
      }
    ...
    const loop = (now: number) => {
      ...
      if (player.hp <= 0 && !adminStateRef.current.godMode) {
        ...
        ctx.fillStyle = "#FFD700"; ctx.font = "bold 48px sans-serif"; ctx.textAlign = "center"; ctx.fillText("DETHRONED!", W / 2, H / 2 - 30);
        ctx.fillStyle = "#fff"; ctx.font = "24px sans-serif"; ctx.fillText(`Hill Time: ${Math.floor(playerHillTime)}s • Score: ${score}`, W / 2, H / 2 + 20);
      ...
      // Hill control logic
      const playerInHill = Math.hypot(player.x - hillCenter.x, player.y - hillCenter.y) < hillRadius;
      const enemiesInHill = enemies.filter(e => Math.hypot(e.x - hillCenter.x, e.y - hillCenter.y) < hillRadius);
      
      if (playerInHill && enemiesInHill.length === 0) {
        playerHillTime += dt; setHillTime(Math.floor(playerHillTime)); setHillOwner("player");
        // Score for holding hill
        if (Math.floor(playerHillTime) % 5 === 0 && Math.floor(playerHillTime) > Math.floor(playerHillTime - dt)) {
          setScore(prev => { const ns = prev + 5; player.score = ns; return ns; });
        }
      } else if (!playerInHill && enemiesInHill.length > 0) {
        enemyHillTime += dt; setHillOwner("enemy");
      } else {
        setHillOwner(playerInHill ? "contested" as any : "none");
      }

      // Bullets
      for (let i = bullets.length - 1; i >= 0; i--) { const b = bullets[i]; b.x += b.vx * dt; b.y += b.vy * dt; b.life -= dt; if (b.life <= 0 || b.x < -50 || b.x > W + 50 || b.y < -50 || b.y > H + 50) { bullets.splice(i, 1); continue; } for (let j = enemies.length - 1; j >= 0; j--) { const e = enemies[j]; if ((b.x - e.x) ** 2 + (b.y - e.y) ** 2 <= (b.r + e.r) ** 2) { e.hp -= b.dmg; e.stun = 0.6; spawnParticles(b.x, b.y, "#FFF3D6", 8); if (e.hp <= 0) { spawnParticles(e.x, e.y, e.color, 16); setScore(prev => { const ns = prev + 10; player.score = ns; return ns; }); setKills(prev => prev + 1); enemies.splice(j, 1); } bullets.splice(i, 1); break; } } }
      ...
      // Enemies AI - they move toward the hill
      for (const e of enemies) {
        if (e.stun > 0) { e.stun -= dt; continue; }
        // Enemies prioritize hill, but also chase player if nearby
        const dToPlayer = Math.hypot(player.x - e.x, player.y - e.y);
        const targetX = dToPlayer < 200 ? player.x : hillCenter.x + rand(-30, 30);
        const targetY = dToPlayer < 200 ? player.y : hillCenter.y + rand(-30, 30);
        ...
        if (dToPlayer < 350 && time - e.lastShot >= 3.0) { e.lastShot = time; const ang = Math.atan2(player.y - e.y, player.x - e.x); enemyBullets.push({ x: e.x, y: e.y, vx: Math.cos(ang) * 200, vy: Math.sin(ang) * 200, r: 6, life: 3, dmg: 10, color: "#FFAA44" }); }
      }

      // Respawn enemies
      if (time - lastSpawn > 4 && enemies.length < 10) { lastSpawn = time; spawnEnemy(); }
      ...
      ctx.save(); ctx.globalAlpha = 0.06; ctx.strokeStyle = "#FFD700";
      ...
      // Hill zone
      ctx.save();
      const hillColor = hillOwner === "player" ? "rgba(68, 136, 255, 0.2)" : hillOwner === "enemy" ? "rgba(255, 68, 68, 0.2)" : "rgba(255, 215, 0, 0.1)";
      ctx.fillStyle = hillColor; ctx.beginPath(); ctx.arc(hillCenter.x, hillCenter.y, hillRadius, 0, Math.PI * 2); ctx.fill();
      ctx.strokeStyle = hillOwner === "player" ? "#4488FF" : hillOwner === "enemy" ? "#FF4444" : "#FFD700";
      ctx.lineWidth = 3; ctx.setLineDash([8, 4]); ctx.stroke(); ctx.setLineDash([]);
      ctx.fillStyle = "#FFD700"; ctx.font = "bold 14px sans-serif"; ctx.textAlign = "center"; ctx.fillText("👑 HILL", hillCenter.x, hillCenter.y - hillRadius - 10);
      ctx.restore();

      // Hill time progress bar
      ctx.save();
      ctx.fillStyle = "rgba(0,0,0,0.5)"; ctx.fillRect(W / 2 - 120, 8, 240, 30);
      ctx.fillStyle = "#333"; ctx.fillRect(W / 2 - 115, 13, 230, 20);
      ctx.fillStyle = "#4488FF"; ctx.fillRect(W / 2 - 115, 13, 230 * Math.min(1, playerHillTime / winTime), 20);
      ctx.fillStyle = "#fff"; ctx.font = "bold 12px sans-serif"; ctx.textAlign = "center";
      ctx.fillText(`👑 ${Math.floor(playerHillTime)}s / ${winTime}s`, W / 2, 27);
      ctx.restore();
      ...
      for (const e of enemies) { ctx.save(); ctx.translate(e.x, e.y); ctx.fillStyle = e.color; ctx.beginPath(); ctx.arc(0, 0, e.r, 0, Math.PI * 2); ctx.fill(); const hpW = 28; ctx.fillStyle = "#333"; ctx.fillRect(-hpW / 2, -e.r - 12, hpW, 6); ctx.fillStyle = "#FFD700"; ctx.fillRect(-hpW / 2, -e.r - 12, hpW * Math.max(0, e.hp / e.maxHp), 6); ctx.restore(); }
  ...
  return (
    <div className="relative">
      <div className="fixed left-4 top-4 bg-card/80 backdrop-blur-sm border border-border rounded-lg p-4 space-y-2">
        <div className="font-bold text-lg text-yellow-400">👑 King of the Hill</div>
      ...
      <div className="fixed right-4 top-4 bg-card/80 backdrop-blur-sm border border-yellow-500/30 rounded-lg p-4 space-y-3 min-w-[180px]">
        <div className="text-center text-yellow-400 font-bold">👑 KOTH</div>
        <div className="text-center text-sm">
          <span className={hillOwner === "player" ? "text-blue-400 font-bold" : hillOwner === "enemy" ? "text-red-400 font-bold" : "text-muted-foreground"}>
            {hillOwner === "player" ? "YOU OWN THE HILL" : hillOwner === "enemy" ? "ENEMY ON HILL" : "HILL UNCLAIMED"}
          </span>
        </div>
        <div className="flex justify-between items-center"><span className="text-sm text-muted-foreground">Hill Time</span><span className="font-bold text-lg text-yellow-400">{hillTime}s</span></div>
        ...
        <div className="w-full bg-secondary rounded-full h-3 overflow-hidden"><div className="bg-gradient-to-r from-yellow-600 to-yellow-400 h-full transition-all duration-300" style={{ width: `${(health / maxHealth) * 100}%` }} /></div>
